...
import { applyPrefix, generateBranchName, sanitizeBranchName, sanitizePrefix } from "./branch-name"
...
export class WorktreeManager {
  ...
  constructor(root: string, log: (msg: string) => void, ops?: GitOps, prefix = "kilo-worktree") {
    this.root = root
    this.dir = path.join(root, KILO_DIR, "worktrees")
    this.git = simpleGit(root)
    this.ops = ops
    this.log = log
    this.prefix = sanitizePrefix(prefix) || "kilo-worktree"
  }
  ...
  private async createWorktreeImpl(params: {
    ...
    if (params.existingBranch) {
      ...
      const existing = await this.git
        ...
        .then((b) => b.all)
        .catch(() => [] as string[])
      branch = applyPrefix(generateBranchName(params.prompt || "agent-task", existing), this.prefix)
    }
